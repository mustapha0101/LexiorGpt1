...
CHAT_PLANNER_SUPPLEMENT = (
    ...
    "SCHÉMAS À NE PAS CONFONDRE : search_legal_documents accepte les champs "
    "déclarés dans SON schéma, notamment search_language, doc_type, "
    "sort_results et size lorsqu'ils sont pertinents. "
    "search_quebec_jurisprudence accepte UNIQUEMENT les champs déclarés dans "
    "SON propre schéma; dans le catalogue actuel, son appel est exactement "
    "{\"query\": \"...\"}. Ne lui envoie jamais search_language, doc_type, "
    "sort_results, size, dataset ou search_type, sauf si son schéma affiché "
    "dans le catalogue les déclare explicitement.\n"
    "Le résumé renvoyé par ce serveur est un CANDIDAT, jamais une preuve : "
...
CHAT_WRITER_SUPPLEMENT = (
    ...
    "défaut.\n"
    "COUVERTURE JURISPRUDENCE : les décisions des tribunaux québécois "
    "(QCCA, QCCS, QCCQ, QCTAL, QCTAT) sont accessibles par "
    "search_quebec_jurisprudence — cherche avant de conclure quoi que ce "
    "soit sur leur disponibilité. La formulation décide du résultat : "
    "nomme le tribunal et emploie le mot « décision » (ex. « Tribunal "
    "administratif du logement décision résiliation de bail pour "
    "non-paiement »); une formulation doctrinale ramène surtout des lois "
    "et des règlements. Si la recherche ne ramène aucune décision après "
    "une reformulation, dis que tu n'en as pas trouvé — pas que le corpus "
    "n'en contient pas — et oriente vers https://www.canlii.org/fr/qc/ "
    "(recherche gratuite) ou SOQUIJ en suggérant des mots-clés. EXCEPTION "
    "à la règle des URL : ces deux portails d'orientation sont permis même "
...
def planner_system_prompt(catalog: ToolCatalog) -> str:
    ...
    return (
        ...
        '  "legal_terms": "LA SITUATION décrite dans le vocabulaire du Code, '
        "5 à 15 mots-clés. « mise en demeure » pour « lettre "
        "d'avertissement », « autorité parentale » pour « mon gars ». "
        "Couvre la relation ou le recours en cause, l'événement et son état "
        "temporel (en cours, achevé, futur), l'effet recherché, puis les "
        "conditions juridiques qui doivent être établies. Une simple liste "
        "d'objets ou de personnes est insuffisante. "
        "Ce sont des CONCEPTS : jamais de numéro d'article, jamais de "
        ...
        "Ne cherche pas automatiquement dans la jurisprudence : récupère d'abord "
        "la législation ou le règlement officiel, puis vérifie sa suffisance.\n\n"
        "  TEMPS 2 — Comprendre l'application :\n"
        "    Suis les renvois normatifs explicites et compare les propositions "
        "du texte aux faits; une réponse conditionnelle utile est prioritaire.\n\n"
        "  TEMPS 3 — Synthétiser :\n"
        "    Cherche une décision seulement si l'utilisateur la demande ou si "
        "une lacune juridique explicite le justifie.\n\n"
        "▸ TOURS SUIVANTS :\n"
        ...
        "  CHOISIR ENTRE LES DEUX RECHERCHES :\n"
        "  • situation décrite avec les mots de tous les jours (« mon chien a\n"
        "    mordu quelqu'un », « mon voisin passe sur mon terrain ») →\n"
        "    semantic_search_ccq. TOUJOURS lui d'abord.\n"
        "  • terme juridique dont tu es sûr qu'il figure TEL QUEL dans le\n"
        "    texte de loi (« servitude », « usufruit ») → search_ccq_keywords.\n"
        "  search_ccq_keywords cherche les mots LITTÉRALEMENT dans l'article.\n"
        "  Le Code écrit « animal », pas « chien » ; « préjudice », pas\n"
        "  « blessure ». Chercher les mots de l'usager en mot-clé ne rend\n"
        "  rien. Si une recherche par mot-clé revient vide, passe à\n"
        "  semantic_search_ccq — ne la relance pas avec un synonyme.\n\n"
        ...
        "  - search_quebec_jurisprudence → get_quebec_regulation\n"
        "    (texte de la décision, par URL du résultat)\n\n"
...
TRAJECTORY_ANSWER_SYSTEM = (
    ...
    "  Pour les questions de fond, réponds d'abord à l'objectif de l'utilisateur, "
    ...
    "Pour toute condition juridique, reprends fidèlement la portée et le "
    "vocabulaire opérants du texte : n'ajoute jamais un critère, une "
    "qualification ou une exception provenant de ta mémoire, même comme "
    "synonyme.\n\n"
    "Règles strictes :\n"
    ...
    "- pour une expression de recours ambiguë (par exemple « porter plainte » "
    "ou « poursuivre »), suis le champ remedy_intent du contrat : distingue "
    "le recours civil couvert des démarches policières, municipales ou "
    "administratives non couvertes ; ne dis jamais qu'une plainte permet "
    "d'obtenir une indemnisation sans preuve distincte ;\n"
    "- les conseils pratiques (photos, documents, témoins, communications) "
    "restent des étapes de conservation des faits, pas des affirmations "
    "juridiques ni une procédure non sourcée ;\n"
    "- lorsqu'une blessure ou un dommage corporel est en cause, adapte ces "
    "étapes aux faits présents : photos, notes médicales, factures, témoins, "
